...
import { KIDS_HEALTH_SERVICES, getKidsHealthService } from "../../kids-health-services";
...
export function generateStaticParams() {
  return KIDS_HEALTH_SERVICES.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const service = getKidsHealthService(slug);
  if (!service) return {};
  return {
...
export default async function KidsHealthServicePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const service = getKidsHealthService(slug);
  if (!service) notFound();
  ...
  return (
    <div className="bg-[#F7F4EE] min-h-screen font-sans">
        ...
        <div className="max-w-[1000px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-10">
          ...
          <div className="lg:sticky lg:top-24 h-fit">
            <div className="bg-white rounded-2xl p-6 border border-[#543826]/10 shadow-lg">
              <p className="text-[#6B7280] text-xs font-semibold uppercase tracking-widest mb-3">Price</p>
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-[#1a2e28] text-3xl font-bold">AED {service.price}</span>
                {service.originalPrice && (
                  <span className="text-[#6B7280] text-base line-through">AED {service.originalPrice}</span>
                )}
              </div>
              {service.offBadge && (
      ...
      {/* ── MORE FROM KIDS' HEALTH ── */}
      {otherServices.length > 0 && (
        <section className="py-12 sm:py-16 px-6 lg:px-8 bg-white">
          <div className="max-w-[1000px] mx-auto">
            <p className="text-[#2D5B4F] text-xs font-semibold uppercase tracking-widest mb-3">More From Kids&rsquo; Health</p>
            <h2 className="text-2xl sm:text-3xl font-bold text-[#1a2e28] mb-8">Other services in this sale</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              {otherServices.map((s) => (
                <Link key={s.slug} href={`/back-to-school-sale/kids-health/${s.slug}`} className="group relative rounded-2xl overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl" style={{ minHeight: "220px" }}>
                  {s.gradient ? (
                  ...
                  <div className="relative z-10 mt-auto p-4">
                    ...
                    <p className="text-white/70 text-xs font-semibold mt-1.5">AED {s.price}</p>
                  </div>
                </Link>
              ))}
            </div>
