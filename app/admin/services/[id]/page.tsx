...
export default function ServiceDetailPage() {
  ...
  const toggleFeatured = async () => {
    try {
      ...
      setService((prev) =>
        prev ? { ...prev, isFeatured: data.data.isFeatured } : prev
      );
    } catch {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <button
          ...
          className="text-[#543826] hover:underline mt-2"
        >
          Back to Services
        </button>
      </div>
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="max-w-5xl mx-auto mb-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            >
              <Star size={16} fill={service.isFeatured ? "currentColor" : "none"} />
              {service.isFeatured ? "Featured" : "Feature"}
            </button>
            <button
              onClick={() => router.push(`/admin/services/edit/${id}`)}
              className="flex items-center gap-2 px-4 py-2 bg-[#543826] text-white rounded-lg text-sm font-medium hover:bg-[#3e2a1c] transition"
            >
              <Pencil size={16} />
              Edit
            </button>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="bg-white rounded-2xl shadow-md overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
            ...
            <div className="p-6 flex flex-col justify-between">
              <div>
                {/* Category badge */}
                {service.category && (
                  <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-[#543826]/10 text-[#543826] mb-3">
                    {service.category}
                  </span>
                )}
                ...
                <div className="flex items-baseline gap-3 mb-4">
                  {service.discountPrice ? (
                    <>
                      <span className="text-2xl font-bold text-[#543826]">
                    ...
                    <span className="text-gray-400 text-sm">No price set</span>
                  )}
                </div>
          ...
          {/* Tabbed Info: Key Benefits / Key Ingredients / Disclaimer */}
          {(service.keyBenefits?.length || service.keyIngredients?.length || service.disclaimer) && (
            ...
            <div className="border-t px-6 py-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {subServices.map((sub, i) => (
                  <div
                    key={i}
                    className="border border-gray-200 rounded-xl p-4 hover:border-[#543826]/30 transition"
                  >
                    <p className="font-medium text-gray-800">{sub.name}</p>
                    <div className="flex items-baseline gap-2 mt-1">
                      <span className="text-[#543826] font-bold">
                        AED {sub.price}
                      </span>
                      {sub.discountPercent && (
...
function ServiceTabs({ service }: { service: Service }) {
  ...
  return (
    <div className="border-t px-6 py-6">
      ...
      {/* Tab content */}
      {activeTab === "benefits" && service.keyBenefits && (
      ...
      {activeTab === "ingredients" && service.keyIngredients && (
        <ul className="space-y-2">
          {service.keyIngredients.map((ing, i) => (
            <li key={i} className="flex items-start gap-2 text-sm text-gray-700">
              <span className="text-[#543826] mt-0.5">&#8226;</span>
              {ing}
            </li>
          ))}
        </ul>
      ...
      {activeTab === "disclaimer" && service.disclaimer && (
